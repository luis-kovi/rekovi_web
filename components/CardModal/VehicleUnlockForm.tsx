...
interface VehicleUnlockFormProps {
  onUnlock: (photos: Record<string, File>, observations?: string) => Promise<void>
  onClose: () => void
}

export default function VehicleUnlockForm({
  onUnlock,
  onClose,
}: VehicleUnlockFormProps) {
  ...
  const handleUnlockVehicle = async () => {
    ...
    try {
      ...
      await onUnlock(photosToUpload, unlockObservations)
            ...
            <div key={photo.key} className="space-y-2">
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-2 text-center hover:border-blue-400 transition-colors">
                ...
                <input
                  ...
                  className="hidden"
                  id={`unlock-upload-${photo.key}`}
                />
                <label
                  htmlFor={`unlock-upload-${photo.key}`}
